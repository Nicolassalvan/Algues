...
def update_mort(population):
    ...
    for i in range(population.nombre_algues-1):
        if(population[i].aggregat==False and np.random.binomial(1,settings.STRESS,1)==1):
            tab_mort=np.append(tab_mort,i)
    population.supprimer_algue(tab_mort)